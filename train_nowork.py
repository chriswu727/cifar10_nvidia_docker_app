...
from app.model import get_model  # Update import path
...
# Enhanced data transformations for better training
...
# Initialize model using the same structure
model = get_model()
...
for epoch in range(25):  # 25 epochs
    ...
    for i, data in enumerate(trainloader, 0):
        ...
        if i % 50 == 49:    # Changed from 100 to 50 for more frequent updates
            avg_loss = running_loss / 50
            losses.append(avg_loss)
            print(f'[{epoch + 1}, {i + 1:5d}] loss: {avg_loss:.3f}')
            running_loss = 0.0
    
    # Calculate accuracies every epoch
